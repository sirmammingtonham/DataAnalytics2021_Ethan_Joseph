...
bronx1$ADDRESSONLY <- gsub(
    "[,][[:print:]]*", "", gsub("[ ]+", "", gsub("^\\s+|\\s+$", "", bronx1$ADDRESS))
)
...
querylist <- geocode(addrlist) # need a google api key, so this lab is broken below here i guess
